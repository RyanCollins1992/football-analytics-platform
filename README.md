...
This README documents **Phases 1-10 — the full build is complete.** See "Known limitations" and the phase list below for what's real vs. deliberately out of scope.
...
- `src/app/admin/` — real database counts, provider API-key configured/not-set status, and per-league data status (Phase 10; reuses `getDataStatus()` unchanged). Informational only — see "Known limitations" below for why there are no sync-trigger controls.
- `src/app/loading.tsx` / `error.tsx` / `not-found.tsx` — root-level, so every route gets them without repeating the boilerplate per page (Phase 10). `error.tsx` is a Client Component (Next's requirement for error boundaries) and logs via `src/lib/api/logger.ts`.
...
- **Phase 10: polish, testing, performance, documentation** — the spec's final phase, a pass back over the whole app for gaps found by re-reading the current codebase, not a speculative wishlist. **Testing**: added `weighting.test.ts` and `rate-limiter.test.ts` — the only two pure functions under `src/lib` that had no sibling test file (69 tests total now, up from 51). **Performance**: rewrote `getDataStatus()` from ~52 queries (a findUnique + 3 more per enabled league) to 4 grouped queries total regardless of league count, verified to return identical data (Premier League: 20 teams/9 matches, unchanged); merged 3 straggler `await`s in `/dashboard` into its existing `Promise.all`. **Polish**: added root-level `loading.tsx` (Suspense skeleton for every route), `error.tsx` (styled client error boundary with a Try Again button, logs via the existing structured logger), and `not-found.tsx` (styled 404, verified for both a bad id like `/teams/999999999` and a nonexistent route) — Next's bare defaults previously covered all three. Rebuilt `/admin` (the last Phase 1 placeholder) into a real, honest status page: live database record counts, provider API-key configured/not-set status (presence only, never the value), and the same per-league data-status table as `/dashboard` — deliberately information-only, no sync-trigger buttons (spec section 24 also wants those, but wiring external API calls into a Server Action is a separate feature, not "polish," and is called out as such in the page's own copy). **Documentation**: this README now covers all 10 phases, plus a new "Known limitations" section below consolidating the caveats found across Phases 3b/7/8/9/10 in one place. Verified against live data via the dev server: `/admin` shows real counts (13 competitions, 18 matches, 30 predictions, 8 evaluated) and all 3 provider keys as configured. ✅

## Known limitations

Collected in one place rather than scattered across nine phases of commit messages — these are the honest current boundaries of the project, not bugs to silently work around:

- **Historical data is still thin.** Only 1 finished Premier League match and 3 finished Scottish Premiership matches are synced as of Phase 9/10's verification. Most models need real prior results for both teams in a fixture, so most current predictions correctly degrade to 0.00–0.00 rather than a meaningful number (see Phase 9's entry above) — this resolves naturally as `npm run sync:fixtures` pulls in more results, not by changing any prediction code.
- **4 leagues have no free current-season data source.** Segunda División, 2. Bundesliga, Serie B, and Ligue 2 aren't covered by football-data.org's or Sportmonks' free tiers; syncing them throws a clear error rather than silently doing nothing (see `docs/providers.md`).
- **No injuries or match-statistics sync exists yet.** None of the three evaluated providers offer a confirmed free current-injuries source; match-level statistics (shots, possession, cards) need API-Football or another source, and API-Football's free tier is historical-only (2022-2024), not wired for live sync (Phase 3b's correction).
- **`/admin` is informational only.** It shows real counts and status but has no manual sync-trigger controls — running external API syncs from inside a web request is a separate feature this project hasn't built (Phase 10).
- **No player-level analytics.** `Player`/`PlayerAvailability`/`Lineup` exist in the schema (Phase 2) but nothing populates or reads them yet — no provider sync job targets them.
